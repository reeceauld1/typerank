        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          <Tooltip content="No badge equipped">
            <button
              ...
              className={`relative w-full flex flex-col items-center gap-2 rounded-lg border p-4 transition-colors ${cellClass(true, !stats.equippedBadge)}`}
            >
              <div className="w-12 h-12 rounded-full border border-dashed border-[var(--border)]" />
            ...
            return (
              <Tooltip key={badge.id} content={unlocked ? `${badge.name} — Unlocked: ${badge.description}` : `${badge.name} — ${badge.description}`}>
                <button
                  ...
                  className={`relative w-full flex flex-col items-center gap-2 rounded-lg border p-4 transition-colors ${cellClass(unlocked, equipped)}`}
                >
                  {!unlocked && (
                  ...
                  <div className="w-12 h-12 rounded-full flex items-center justify-center bg-[var(--bg-elevated)]" style={{ color: badge.color }}>
