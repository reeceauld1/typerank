-- New "Bug Fixer" badge, granted by hand in the SQL Editor whenever a
-- reported bug turns out to be real and worth fixing — no automatic
-- criteria, same pattern as Supporter/GOAT:
--   update public.user_stats set is_bug_fixer = true where username = 'someusername';
insert into public.badge_catalog (id) values ('bug_fixer')
  on conflict (id) do nothing;

alter table public.user_stats add column if not exists is_bug_fixer boolean not null default false;
